  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-base font-semibold text-gray-800">Equipos e Instalaciones</h2>
                ...
                <CardContent className="p-4 space-y-2">
                  ...
                  {eq.proximo_mantenimiento && (
                    <p className={`text-xs ${vencido ? 'text-amber-600 font-medium' : 'text-gray-400'}`}>
                      🔧 Prox. mant.: {new Date(eq.proximo_mantenimiento + 'T00:00:00').toLocaleDateString('es-CO', { day: '2-digit', month: 'short', year: 'numeric' })}
                    </p>
                  )}
                  {eq.sensor_id
                    ? <p className="text-xs text-blue-600">📡 Sensor: {eq.sensor_id}</p>
                    : <p className="text-xs text-gray-300">📡 Sin sensor IoT</p>
                  }

                  <div className="flex gap-1.5 pt-1">
                    {eq.estado !== 'operativo' && (
                      <button onClick={() => cambiarEstado(eq.id, 'operativo')} className="text-[10px] px-2 py-0.5 rounded-full bg-green-100 text-green-700 hover:bg-green-200">✓ Operativo</button>
                    )}
                    {eq.estado !== 'falla' && (
                      <button onClick={() => cambiarEstado(eq.id, 'falla')} className="text-[10px] px-2 py-0.5 rounded-full bg-red-100 text-red-700 hover:bg-red-200">✗ Falla</button>
                    )}
                    {eq.estado !== 'mantenimiento' && (
                      <button onClick={() => cambiarEstado(eq.id, 'mantenimiento')} className="text-[10px] px-2 py-0.5 rounded-full bg-yellow-100 text-yellow-700 hover:bg-yellow-200">⚙ Mant.</button>
                    )}
                  </div>
                </CardContent>
              </Card>
