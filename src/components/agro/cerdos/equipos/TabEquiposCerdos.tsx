...
type Equipo = Database['public']['Tables']['equipos_cerdos']['Row']

interface Props { loteActual: LoteCerdos }

const TIPO_EQUIPO = ['ventilador', 'comedero', 'bebedero', 'calefactor', 'lampara', 'bomba', 'bascula', 'otro']
const ESTADO_LABELS: Record<string, { label: string; color: string; border: string }> = {
...
export default function TabEquiposCerdos({ loteActual }: Props) {
  ...
  const fetchEquipos = useCallback(async () => {
    setLoading(true)
    const { data } = await supabase.from('equipos_cerdos').select('*')
  ...
  async function handleCrear(e: React.FormEvent) {
    ...
    setSaving(true)
    const { error } = await supabase.from('equipos_cerdos').insert({
  ...
  async function cambiarEstado(id: string, estado: string) {
    const { error } = await supabase.from('equipos_cerdos').update({ estado: estado as Equipo['estado'] }).eq('id', id)
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold text-gray-800">Equipos e Instalaciones</h2>
        ...
        <Card className="border-orange-200">
          <CardContent className="p-4">
            <form onSubmit={handleCrear} className="grid grid-cols-2 md:grid-cols-3 gap-3">
              <div className="space-y-1"><Label className="text-xs">Nombre *</Label><Input placeholder="Ej: Ventilador norte" value={form.nombre} onChange={e => setForm(p => ({ ...p, nombre: e.target.value }))} /></div>
              <div className="space-y-1">
                <Label className="text-xs">Tipo *</Label>
                <Select value={form.tipo} onValueChange={v => setForm(p => ({ ...p, tipo: v ?? '' }))}>
                  <SelectTrigger><SelectValue placeholder="Tipo" /></SelectTrigger>
                  <SelectContent>{TIPO_EQUIPO.map(t => <SelectItem key={t} value={t}>{t.charAt(0).toUpperCase() + t.slice(1)}</SelectItem>)}</SelectContent>
                </Select>
              </div>
              <div className="space-y-1">
                <Label className="text-xs">Estado</Label>
                <Select value={form.estado} onValueChange={v => setForm(p => ({ ...p, estado: v ?? 'operativo' }))}>
              ...
              <div className="space-y-1"><Label className="text-xs">Ubicación</Label><Input placeholder="Corral A, Sección norte..." value={form.ubicacion} onChange={e => setForm(p => ({ ...p, ubicacion: e.target.value }))} /></div>
              ...
              <div className="space-y-1"><Label className="text-xs">Número de serie</Label><Input value={form.numero_serie} onChange={e => setForm(p => ({ ...p, numero_serie: e.target.value }))} /></div>
              <div className="col-span-2 md:col-span-3 space-y-1"><Label className="text-xs">Observaciones</Label><Input value={form.observaciones} onChange={e => setForm(p => ({ ...p, observaciones: e.target.value }))} /></div>
              <div className="col-span-2 md:col-span-3 flex justify-end">
                <Button type="submit" disabled={saving} className="bg-green-700 hover:bg-green-800 text-white">{saving ? 'Guardando...' : 'Registrar equipo'}</Button>
              </div>
            </form>
          </CardContent>
        </Card>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {equipos.map(eq => {
            const est = ESTADO_LABELS[eq.estado] ?? ESTADO_LABELS.inactivo
            const vencido = eq.proximo_mantenimiento && eq.proximo_mantenimiento < today
            return (
              <Card key={eq.id} className={`border-2 ${est.border} transition-all`}>
                <CardContent className="p-4 space-y-2">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <div className="flex flex-col items-end gap-1">
                      <Badge className={`${est.color} text-[10px] font-medium`}>
                  ...
                  {eq.proximo_mantenimiento && (
                    <p className={`text-xs ${vencido ? 'text-amber-600 font-medium' : 'text-gray-400'}`}>
                      <Ic n="herramienta" /> Prox. mant.: {new Date(eq.proximo_mantenimiento + 'T00:00:00').toLocaleDateString('es-CO', { day: '2-digit', month: 'short', year: 'numeric' })}
                    </p>
                  )}
                  {eq.sensor_id
                    ? <p className="text-xs text-blue-600"><Ic n="sensor" /> Sensor: {eq.sensor_id}</p>
                    : <p className="text-xs text-gray-300"><Ic n="sensor" /> Sin sensor IoT</p>
                  }

                  <div className="flex gap-1.5 pt-1">
                    {eq.estado !== 'operativo' && (
                      <button onClick={() => cambiarEstado(eq.id, 'operativo')} className="text-[10px] px-2 py-0.5 rounded-full bg-green-100 text-green-700 hover:bg-green-200"><Ic n="check" /> Operativo</button>
                    )}
                    {eq.estado !== 'falla' && (
                      <button onClick={() => cambiarEstado(eq.id, 'falla')} className="text-[10px] px-2 py-0.5 rounded-full bg-red-100 text-red-700 hover:bg-red-200"><Ic n="x" /> Falla</button>
                    )}
                    {eq.estado !== 'mantenimiento' && (
                      <button onClick={() => cambiarEstado(eq.id, 'mantenimiento')} className="text-[10px] px-2 py-0.5 rounded-full bg-yellow-100 text-yellow-700 hover:bg-yellow-200"><Ic n="ajustes" /> Mant.</button>
                    )}
                  </div>
                </CardContent>
              </Card>
      ...
      <Card className="border-dashed border-gray-300 bg-gray-50">
        <CardContent className="p-4 text-center space-y-1">
          ...
          <p className="font-medium text-gray-600 text-sm">Automatización Inteligente de Porqueriza — Próximamente</p>
          <p className="text-xs text-gray-400">Control remoto de ventiladores, comederos automáticos y báscula dinámica. Protocolos MQTT y Modbus.</p>
          <div className="flex justify-center gap-2 mt-2">
            {['LoRaWAN', 'Modbus RTU', 'Alertas SMS'].map(b => (
